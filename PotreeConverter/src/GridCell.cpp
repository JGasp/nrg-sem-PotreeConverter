...
GridCell::GridCell(bool isQuadTree){
	this->isQuadTree = isQuadTree;
}

GridCell::GridCell(SparseGrid *grid, GridIndex &index, bool isQuadTree){
	this->isQuadTree = isQuadTree;
	this->grid = grid;	
	...
	for(int i = max(index.i -1, 0); i <= min(grid->width-1, index.i + 1); i++){
		for(int j = max(index.j -1, 0); j <= min(grid->height-1, index.j + 1); j++){
			...
			if (isQuadTree) {
				long long key = ((long long)j << 20) | i;
...
bool GridCell::isDistant(const Vector3<double> &p, const double &squaredSpacing) const {
	for(const Vector3<double> &point : points){
		...
		double distance;
		if (isQuadTree) {
			distance = p.squaredDistanceTo2D(point);
		} else	{
			distance = p.squaredDistanceTo(point);
		}

		if(distance < squaredSpacing){
			return false;
		}
